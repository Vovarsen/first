...
template <class T>
DynamicArr<T>::DynamicArr() : size_(0), capacity_(1) {
    arr_ = new T[capacity_];
}

template <class T>
DynamicArr<T>::DynamicArr(size_t size, T elem) : size_(size), capacity_(coeff_ * size_) {
    arr_ = new T[capacity_];
...
template <class T>
DynamicArr<T>::DynamicArr(const DynamicArr<T>& rhs) : size_(rhs.size_), capacity_(rhs.capacity_) {
    arr_ = new T[capacity_];
    for (size_t i = 0; i < size_; ++i) {
        arr_[i] = rhs.arr_[i];
    }
...
DynamicArr<T>::DynamicArr(DynamicArr<T>&& rhs) noexcept :
    ...
    // Установим ресурсы исходного объекта в nullptr
    rhs.arr_ = nullptr;
    rhs.size_ = 0;
    rhs.capacity_ = 0;
}
...
DynamicArr<T>& DynamicArr<T>::operator=(DynamicArr<T>&& rhs) noexcept {
    if (this != &rhs) {
        delete[] arr_; // Освобождаем свои ресурсы
        arr_ = rhs.arr_; // Перемещаем данные
        size_ = rhs.size_;
        capacity_ = rhs.capacity_;
